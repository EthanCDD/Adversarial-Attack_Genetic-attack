...
from genetic_stop_words import GeneticAttack_pytorch
...
#    train_set = Subset(data_set, indx)
    ...
    lstm_size = 128
    rnn_state_save = os.path.join(file_path,'best_lstm_0.7_0.001_350')
    ...
    for order, (seq, l, target) in enumerate(test_loader):
      ...
      if order>=order_pre:
        ...
        print('Attack success rate : {:.2f}%'.format(np.mean(successful_attacks)*100))
        ## Save success
    with open('attack_results_final_0003.pkl', 'wb') as f:
        pickle.dump((orig_list, adv_list, normalized_dist_list), f)
