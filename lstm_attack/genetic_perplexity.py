...
class GeneticAttack_pytorch(object):
  def __init__(self, model, batch_model, neighbour_model, dist,
               gpt_lm, max_iters, dataset,
  ...
  def attack(self, seq, target, l, max_change = 0.4):
    ...
    tmp = [glove_utils.pick_most_similar_words(seq[i], self.dist, ret_count = 50, threshold = 0.5) for i in range(seq_len)]
    ...
    tmp = [glove_utils.pick_most_similar_words(
        seq[i], self.dist, self.top_n1, 0.5
    ) for i in range(seq_len)]
    neighbour_list = [t[0] for t in tmp]
    neighbour_dist = [t[1] for t in tmp]
