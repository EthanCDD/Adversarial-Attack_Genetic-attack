...
def train():
    ...
    # pytorch
    max_len = 150
    with open('attack_results_final_150.pkl', 'rb') as f:
    ...
    for i in range(len(results[1])):
        if np.array(results[1][i]).shape == ():
            continue
        seqs.append(results[1][i].tolist())
        lens.append(results[2][i])
        tgts.append(results[3][i])

    dataset.train_seqs2.extend(seqs)
    ...
    best_save_path = os.path.join(save_path, 'best_defense_'+str(kept_prob)+'_'+str(learning_rate)+'_'+str(max_len))
