...
#    all_train_set = Subset(data_set, indx)
    train_indx = random.sample(indx, int(num_train*0.8))
    vali_indx = [i for i in indx if i not in train_indx]
    train_set = Subset(data_set, train_indx)
    vali_set = Subset(data_set, vali_indx)
    ...
    best_save_path = os.path.join(save_path, 'best_lstm_'+str(kept_prob)+'_'+str(learning_rate))
