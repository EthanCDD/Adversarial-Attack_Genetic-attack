import os
import sys
sys.path.append('Adversarial-Attack_Genetic-attack')
...
def run():
    ...
    lstm_size = 128
    rnn_state_save = os.path.join(save_path,'best_bert_0.7_0.001_bert_300')
    ...
    n_prefix = 6
    n_suffix = 6
    batch_model = bert_lstm(bert, 2, False, nlayer, lstm_size, True, 0.7)#SentimentAnalysis(batch_size=pop_size, embedding_matrix = embedding_matrix, hidden_size = lstm_size, kept_prob = 0.73, num_layers=2, bidirection=True)

    batch_model.eval()
    batch_model.load_state_dict(torch.load(rnn_state_save))
    batch_model.to(device)

    neighbour_model = bert_lstm(bert, 2, False, nlayer, lstm_size, True, 0.7)#SentimentAnalysis(batch_size=batch_size, embedding_matrix = embedding_matrix, hidden_size = lstm_size, kept_prob = 0.73, num_layers=2, bidirection=True)
...
#           break 
    ...
    for order, (seq, l, target) in enumerate(test_loader_bert):
      ...
      if order>=order_pre:
        ...
        model.eval()
        with torch.no_grad():
          prediction = model.pred(seq, l, False)[1].cpu().detach().numpy()
        ...
        x_adv, seq_out = ga_attack.attack( seq, target, l.type(torch.LongTensor))
        ...
        if x_adv is None:
            ...
            w_be = [seq_out[i] for i in list(np.where(np.array(seq_out) != np.array(x_adv))[0])]
            w_to = [x_adv[i] for i in list(np.where(np.array(seq_out) != np.array(x_adv))[0])]
            for i in range(len(w_be)):
              print('{} ----> {}'.format(w_be[i], w_to[i]))
        print('--------------------------')
