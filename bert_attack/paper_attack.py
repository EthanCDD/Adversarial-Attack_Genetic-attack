...
class GeneticAtack(object):
    ...
    def select_best_replacement(self, pos,  x_cur, x_orig,x_len, target, replace_list):
        ...
        with torch.no_grad():
            new_x_preds = self.neighbour_model(new_seq_list_tensor, l_tensor, False)[1].cpu().detach().numpy()
        ...
        self.model.eval()
        with torch.no_grad():
          orig_score = self.model(seq_tensor, l_tensor, False)[1].cpu().detach().numpy()[0, target]
    ...
    def perturb(self, x_cur, x_orig, x_len, neigbhours, neighbours_dist,  w_select_probs, target):
        ...
        return self.select_best_replacement(rand_idx, x_cur, x_orig, x_len, target, replace_list)

    def generate_population(self, x_orig, x_len, neigbhours_list, neighbours_dist, w_select_probs, target, pop_size):
        return [self.perturb(x_orig, x_orig, x_len, neigbhours_list, neighbours_dist, w_select_probs, target) for _ in range(pop_size)]
    ...
    def attack(self, x_orig, target, max_change=0.4):
        ...
        pop = self.generate_population(
            x_orig, x_len, neigbhours_list, neighbours_dist, w_select_probs, target, self.pop_size)
        for i in range(self.max_iters):
            ...
            with torch.no_grad():
              pop_preds = self.batch_model(pop_tensor, l_tensor, False)[1].cpu().detach().numpy()
            ...
            childs = [self.crossover(pop[parent1_idx[i]],
                      ...
                      for i in range(self.pop_size-1)]
            childs = [self.perturb(
                x, x_orig, x_len, neigbhours_list, neighbours_dist, w_select_probs, target) for x in childs]
            pop = elite + childs
